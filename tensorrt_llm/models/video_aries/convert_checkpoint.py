...
def convert_video_dit(args, mapping:Mapping, dtype='bfloat16'):
    weights = dict()
    ...
    with open(os.path.join(args.output_dir, 'config.json'), 'r') as f:
        ...
        for k, v in weights.items():
            if re.match('^model.txt_in.individual_token_refiner.blocks.*.self_attn_qkv.weight$', k):
                ...
                split_qkv = split_qkv_tp(qkv, args.num_heads, args.hidden_size,
                                          tensor_parallel, mapping.tp_rank)
                split_mlp = split(mlp, tensor_parallel, mapping.rank, dim=0)
...
def covert_and_save(args, rank):
    ...
    weights = convert_video_dit(args, mapping, dtype=args.dtype)

    tensorrt_llm.profiler.print_memory_usage(f'Before weights save.', 'MiB')
    tik = time.time()
    safetensors.torch.save_file(
        weights, os.path.join(args.output_dir, f'rank{rank}.safetensors'))
    tok = time.time()
    t = time.strftime('%H:%M:%S', time.gmtime(tok - tik))
    tensorrt_llm.profiler.print_memory_usage(f'After weights save. Total time: {t}', 'MiB')
